...
class MGProductImport
{
	...
	static public function parseProductlist($productlist)
	{
		...
		switch(MGImportSettings::DOCTYPE){
			case MGImportSettings::JSON:
				// Workaround for \\u00 and \u00
				$result = str_replace("\\\\u00", "\\u00", $productlist);
	...
	*/
	static protected function importProductGroup($group)
	{
		...
		$cat->setShortDescription($group->description);
	...
	*/
	static protected function importCategory($category)
	{
		...
		$cat->setShortDescription($category->description);
		$cat->save();
	}	
	...
	*/
	static protected function importProduct($product)
	{
		...
		*/
		...
		$mprod->setShortDescription($product->description);
	...
	*/
	static public function importImageOfProduct($sku)
	{
		...
		/* 
			nation verarschung
				geht nicht zu ende	
				und geht nicht zu ende
				( warum eigentlich keine klassennamen als *magic nummern zurückliefern?
		
			// return des karlie services
			tdClass Object
			(
    				[15044] => stdClass Object
        			(
            				[image] => 5376.png
           				[image_msg] => 5376.png has been created.
           				[status] => 2
        			)

			)

			// na wie sollte ich das parsen?
			// schnell schnell: ich will lösungen 
			// ( wir stellen heute abend um....
			// schwitz: ja, geht es denn nicht, herr berzsinszky, können sie das nicht, dann suche ich mir einen, der das kann....
		...
		*/
		...
		if(false === strrpos($res, ("image data"))){
			$target = MGImportSettings::FANTOMAS;
	...
	*/
	...
	static public function activateImagesByLabel($label)
	{
		MGProductImport::initMagento();
		$coll = Mage::getModel("catalog/product")->getCollection();
		MGProductImport::log("activateItemsByLabel():" . $label . PHP_EOL);
		foreach($coll as $prod){
			$prod = $prod->load($prod->getId());
			$mg = $prod->getMediaGallery();
			foreach($mg["images"] as $image){
