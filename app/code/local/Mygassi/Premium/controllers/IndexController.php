...
// --> checkmo shortcut : without payone	
	...
	public function pcfsAction()
	{
		...
		// email validation
		if(!filter_var($post["p_email"], FILTER_VALIDATE_EMAIL)){
		...
		if(4 > strlen($post["p_zip"])){
			Mage::getSingleton("core/session")->setErrorMessage("1 Bitte &uuml;berpr&uuml;fen Sie die Postleitzahl");
			$this->getResponse()->setHeader("Location", $loc)->sendHeaders();
			exit(1);
		}
		else{
		...
		if(4 > strlen($post["c_zip"])){
			Mage::getSingleton("core/session")->setErrorMessage("2 Bitte &uuml;berpr&uuml;fen Sie die Postleitzahl");
			$this->getResponse()->setHeader("Location", $loc)->sendHeaders();
			exit(1);
		}
		else{
		...
		// c_city
		if(3 > strlen($post["c_city"])){
			Mage::getSingleton("core/session")->setErrorMessage("Bitte &uuml;berpr&uuml;fen Sie die Stadt");
			$this->getResponse()->setHeader("Location", $loc)->sendHeaders();
			exit(1);
		}
		else{
