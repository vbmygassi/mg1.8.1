<?php
class Mygassi_Webshop_RegisterController extends Mage_Checkout_Controller_Action
{
	public function indexAction()
	{
		print Mage::getSingleton("core/layout")
			->createBlock("core/template")
			->setTemplate("web/reg.phtml")
			->toHtml();
	}
	...
	public function regAction()
	{
		...
		if(
			...
			!isset($post["user_pass_1"]) ||
			!isset($post["user_pass_2"]) ||
		...
		// validates pass_1 
		// todo: check mage passwort rules 
		if(8 > strlen($post["user_pass_1"])){
			$this->sendError("Das Passwort ist nicht gültig.");
			exit(1);
		}	

		// validates pass_2 
		// todo: check mage passwort rules 
		if(8 > strlen($post["user_pass_2"])){
			$this->sendError("Das Passwort ist nicht gültig.");
			exit(1);
		}
		// validates pass_1 <-> pass_2
		if($post["user_pass_1"] != $post["user_pass_2"]){
			$this->sendError("Die angegebenen Passwörter stimmen nicht überein.");
			exit(1);
		}	
	...
	protected function sendErrorMessage($message)
	{
		...
		$loc = Mage::getBaseUrl() . "web/register/";
		Mage::app()->getResponse()->setHeader("Location", $loc)->sendHeaders();
	}	
